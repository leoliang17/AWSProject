...
class CreateEmployeeForm(forms.Form):
    ...
    location = forms.ModelChoiceField(label='Location',queryset=models.Location.objects.all(), required=True, to_field_name="name")

class CreateAssetForm(forms.Form):
    id = forms.CharField(label='ID', max_length=10, required=True)
    currentLocation = forms.ModelChoiceField(label='Current Location',queryset=models.Location.objects.all(), required=True, to_field_name="name")
    orgTag = forms.CharField(label='Organization Tag', required=True)
    manufacturer = forms.ModelChoiceField(label='Manufacturer',queryset=models.Manufacturer.objects.all(), required=True, to_field_name="name")
    ...
    employee = forms.ModelChoiceField(label='Employee',queryset=models.Employee.objects.all(), required=True, to_field_name="first_name")
